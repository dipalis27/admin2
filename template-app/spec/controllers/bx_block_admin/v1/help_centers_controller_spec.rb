...
RSpec.describe BxBlockAdmin::V1::HelpCentersController, type: :controller do
  ...
  describe 'Help center' do
    ...
    context '/index' do
      ...
      it 'static page is not present' do
        get :index, params: @request_params
        expectation = HashWithIndifferentAccess.new({"message" => "No static pages found"})
    ...
    context '/create' do
      it 'when admin_user provide all the fields' do
        ...
        post :create, params: {"title": Faker::Lorem.word, 'description': Faker::Lorem.sentence, 'help_center_type': "other"}
        expect(response.status).to eq(200)
      end
      
      it 'when admin_user does not provide all the fields ' do
        request.headers['token'] = @token
        post :create, params: {"title": Faker::Lorem.word, 'help_center_type': "other"}
