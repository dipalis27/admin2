...
module BxBlockOrderManagement
  class AddProduct
    ...
    def initialize(params, user)
      ...
      @order = user.orders.create!(status: "created", coupon_code_id: @params[:coupon_code_id]) unless @params[:cart_id].present?
    ...
    def call
      if params[:catalogue_variant_id].present? && catalogue_variant.blank?
        return OpenStruct.new(success?: false, data: nil, msg: 'Sorry, Product Variant is not found for this product', code: 404)
      elsif product_not_available?
        return OpenStruct.new(success?: false, data: nil, msg: 'Sorry, Product is out of stock', code: 404)
      elsif order.blank?
        return OpenStruct.new(success?: false, data: nil, msg: 'Sorry, cart is not found', code: 404)
      else
    ...
    def order_item_params
      params.permit(:quantity, :subscription_quantity, :catalogue_id, :catalogue_variant_id, :subscription_period, :subscription_package, :preferred_delivery_slot)
    end
