module BxBlockOrderManagement
  class UpdatePayment
    ...
    def call
      @response = BxBlockPayments::HyperpayPaymentGateway.get_status(transaction_id, ("?entityId=#{ENTITYID}"))
      if payment_status == 'success'
        proccess_after_payment if BxBlockApiConfiguration::ApiConfiguration.find_by(configuration_type: '525k').present?
      elsif payment_status == 'pending'
        process_pending_order 'pending_order'
        return OpenStruct.new(success?: false, msg: I18n.t('messages.after_payments.internal_error'), code: 400)
      elsif payment_status == 'failed'
        process_pending_order 'payment_failed'
        return OpenStruct.new(success?: false, msg: I18n.t('messages.after_payments.status_fail'), code: 400)
      else
    ...
    def handle_other_model_attribtues
      ActiveRecord::Base.transaction do
        ...
        OpenStruct.new(success?: false, msg: I18n.t('messages.after_payments.internal_error'), code: 400)
    ...
    def proccess_after_payment
      ...
      if result.success?
        shipment_params = BxBlockFedexIntegration::ShipmentAttributesCreation.new(order, @response).call
    ...
    def process_pending_order req_type
      order.update!(order_date: Time.current, is_gift: params[:is_gift], source: params[:source], schedule_time: schedule_time)
      order.send("#{req_type}!")
      create_transaction
    end
